...
class SessionAwareJwtAuthenticationConverterTest {
    ...
    @Test
    @DisplayName("JWT에 userId가 빈 문자열이면 SessionExpiredException 발생")
    void convert_BlankUserId_ThrowsSessionExpiredException() {
        // Given
        Jwt jwtWithBlankUserId = createJwt(TEST_EMAIL, "", TEST_SESSION_ID);

        // When & Then
        SessionExpiredException exception = assertThrows(
            SessionExpiredException.class,
            () -> converter.convert(jwtWithBlankUserId)
        );
    ...
    @Test
    @DisplayName("JWT에 subject(email)가 null이면 인증 이름은 빈 문자열로 정규화됨")
    ...
    @Test
    @DisplayName("빈 sessionId는 예외 발생")
